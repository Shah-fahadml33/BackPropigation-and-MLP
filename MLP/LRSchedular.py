...
class EarlyStopping:
    ...
    def __call__(self,val_loos,model_params):
        if val_loos<self.best_loss-self.min_delta:
            self.counter=0
            self.best_weights={k: np.copy(v) for k,v in model_params.items()}
    ...
    def restore_best_weight(self,model_params):
        if self.best_loss is not None:
            for k in model_params.keys():
                model_params[k]=np.copy(self.best_weights[k])
